...
VIS_DIR = Path(r"D:\projects\polypseg_baseline\runs\boundary_detection_vis")
...
def main():
    ...
    if not image_dir.exists() or not mask_dir.exists():
        raise FileNotFoundError(f"找不到 images/masks 文件夹：{image_dir} / {mask_dir}")

    if not CKPT_PATH.exists():
        raise FileNotFoundError(f"best.pth 不存在：{CKPT_PATH}")
